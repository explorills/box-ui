 ...
 *
 * Cue contract — names referenced by OneBox.tsx:
...
const env = (gain: GainNode, attack: number, peak: number, release: number, t0: number) => {
...
interface ToneOpts {
  f: number; fEnd?: number; dur: number;
  type?: OscillatorType; peak?: number; delay?: number;
  attack?: number; detune?: number;
}
interface NoiseOpts {
  f?: number; fEnd?: number; q?: number; dur: number;
  peak?: number; filter?: BiquadFilterType; delay?: number; attack?: number;
}
interface BellOpts {
  f: number; dur: number; peak?: number; delay?: number;
...
const SFX: Record<string, () => void> = {
  ...
  win: () => {
    ...
    notes.forEach((f, i) => {
      ...
      tone({ f: f * 1.005, dur: 0.5, type: 'sine', peak: 0.06, delay: i * 0.06 });
    });
  ...
  claim: () => {
    ...
    tone({ f: 196, dur: 0.18, type: 'sine', peak: 0.06 });
    vibrate(15);
  },
  merge: () => {
...
export const OneBoxAudio = {
  play: (name: string) => { const fn = SFX[name]; if (fn) fn(); },
